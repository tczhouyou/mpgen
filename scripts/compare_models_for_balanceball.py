...
from run_mdnmp_for_balanceball import train_evaluate_mdnmp_for_balanceball
from run_baselines_for_balanceball import train_evaluate_baseline_for_balanceball
...
if options.version == "v2":
    ...
    data_dir = '../experiments/mujoco/balanceball/balanceball_mpdata'
...
for expId in range(options.expnum):
    ...
    for i in range(len(tsize)):
        ...
        omdn[0, i] = train_evaluate_mdnmp_for_balanceball(mdnmp, trqueries, trvmps, tdata, max_epochs=max_epochs,
                                                          sample_num=sample_num, env_file=env_file,
        ...
        mdnmp.is_normalized_grad = False
        mce[0, i] = train_evaluate_mdnmp_for_balanceball(mdnmp, trqueries, trvmps, tdata, max_epochs=max_epochs,
                                                            sample_num=sample_num, env_file=env_file,
                                                            isdraw=options.isdraw, num_test=options.ntest,
                                                            learning_rate=lrate)
        ...
        omce[0, i] = train_evaluate_mdnmp_for_balanceball(mdnmp, trqueries, trvmps, tdata,max_epochs=max_epochs,
                                                            sample_num=sample_num, env_file=env_file,
                                                            isdraw=options.isdraw, num_test=options.ntest,
                                                            learning_rate=lrate)
        ...
        mdnmp.nll_lrate = lrate
        mdnmp.ent_lrate = lrate
        oelk[0, i] = train_evaluate_mdnmp_for_balanceball(mdnmp, trqueries, trvmps, tdata,max_epochs=max_epochs,
                                                            sample_num=sample_num, env_file=env_file,
                                                            isdraw=options.isdraw, num_test=options.ntest,
                                                            learning_rate=lrate)
        ...
        baseline[0, i] = train_evaluate_baseline_for_balanceball("GPR", trqueries, trvmps, tdata,  sample_num=sample_num,
                                                                 env_file = env_file,
                                                                 isdraw = options.isdraw, num_test = options.ntest)
